...
function Library:create(options)
	...
	if readfile and writefile and isfile then
		if not isfile("HydrinityUISettings.json") then
			writefile("HydrinityUISettings.json", HTTPService:JSONEncode(settings))
		end
		settings = HTTPService:JSONDecode(readfile("HydrinityUISettings.json"))
	...
	local toggleui = settingsTab:keybind{
		Name = "Toggle Key",
        ...
        KeyChanged = function(newKey)
            updateSettings("ToggleKey", newKey.Name)
        end
	}

	local dragtoggle = settingsTab:toggle{
		Name = "Lock Dragging",
		Description = "Makes sure you can't drag the UI outside of the window.",
		StartingState = settings.DragLock,
		Callback = function(state)
			Library.LockDragging = state
            updateSettings("DragLock", state)
		end,
	}

	local dragslider = settingsTab:slider{
		Name = "UI Drag Speed",
...
function Library:dropdown(options)
	options = self:set_defaults({
		...
		OnOpenCallback = function() return end
	}, options)
	...
	local selectedText = dropdownContainer:object("TextBox", {
		AnchorPoint = Vector2.new(1, 0),
		Theme = {
		...
		TextSize = 14,
		Text = "",
        PlaceholderText = options.StartingText
	...
	do
		...
		toggle = function(state)
			...
			if open then
				...
				-- itemContainer:tween{Size = UDim2.new(1, -10, 0, #items == 1 and newSize or 0)}
				-- dropdownContainer:tween({Size = UDim2.new(1, -20, 0, #items == 1 and 52 + newSize or 52)}, function()
				-- 	self:_resize_tab()
				-- end)
				-- icon:tween{Rotation = 0, Position = UDim2.new(1, -11, 0, 12)}
			end
        ...
        selectedText.Focused:Connect(function()
			if not open then
            	toggle(true)
			end
        end)

        selectedText.AbsoluteObject:GetPropertyChangedSignal("Text"):Connect(function()
			if not open then
				return
			end
            local count = 0
            for i, item in pairs(itemContainer.AbsoluteObject:GetChildren()) do
                if not item:IsA("GuiObject") then
                    continue
                end
                local name = string.lower(item.Name)
                local search = string.lower(selectedText.Text)
                local found = string.find(name, search)
                item.Visible = string.find(name, search)
                count = string.find(name, search) and count + 1 or count
            end

            local newSize = (25 * count) + 5
            itemContainer:tween{Size = UDim2.new(1, -10, 0, newSize)}
            dropdownContainer:tween({Size = UDim2.new(1, -20, 0, 52 + newSize)}, function()
                self:_resize_tab()
            end)
...
function Library:slider(options)
	...
	function methods:Set(value)
		if options.Max - options.Min <= 0 then
	...
	function methods:SetMax(value)
		options.Max = value
		local oldNum = tonumber(valueText.Text)
		if oldNum > options.Max then
			oldNum = options.Max
